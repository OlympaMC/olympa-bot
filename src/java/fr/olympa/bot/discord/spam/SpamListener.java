...
public class SpamListener extends ListenerAdapter {
	...
	public void onGuildMessageReceived(GuildMessageReceivedEvent event) {
		...
		List<Member> mentionedMembers = message.getMentionedMembers().stream().filter(m -> !m.getUser().isBot() && !m.getUser().isFake() && member.getPermissions(channel).contains(Permission.MESSAGE_READ)).collect(Collectors.toList());
