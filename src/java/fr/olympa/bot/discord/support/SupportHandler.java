...
public class SupportHandler {
	...
	public static void createChannel(Category category, Member member) {
		DiscordUtils.deny(category, null, perm -> {
			category.createTextChannel(member.getEffectiveName().toLowerCase()).queue(textChannel -> {
				...
				DiscordUtils.allow(textChannel, member, Permission.MESSAGE_READ);
	...
	public static boolean panelStaff(ReactionEmote reactionEmote, TextChannel textChannel, long msgId, Member member) {
		...
		Permission[] perms = new Permission[] { Permission.MESSAGE_READ, Permission.MESSAGE_MENTION_EVERYONE, Permission.MESSAGE_ADD_REACTION };
		if (staff.isEmpty()) {
		...
		} else if (staff.stream().map(Member::getOnlineStatus).filter(s -> s != OnlineStatus.OFFLINE && s != OnlineStatus.INVISIBLE).findFirst().isPresent())
